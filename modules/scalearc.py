...
def parse(s):
    ...
    if command == "events":
        ...
        s.do_BAD_HEAD()
        s.wfile.write("ERROR: invalid command given: " + command + "\n") 
        return;
...
def doCall(s, url):
    ...
    try:
        ...
        s.do_BAD_HEAD()
        return False
    if json_content["success"] != 1:
        s.do_BAD_HEAD()
        s.wfile.write("ERROR: data fetch failed: %s \n" % content)
        return False
...
def eventcheck(s, content):
    if content["data"]["total"] == 0:
        ...
        s.wfile.write("%s\n" % s.path)
        s.wfile.write("OK: No New Event In ScaleArc\n\n")
        return
    else:
        s.do_BAD_HEAD()
